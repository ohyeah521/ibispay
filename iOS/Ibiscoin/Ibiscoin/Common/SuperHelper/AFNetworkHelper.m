...
//上传或者修改1张图片
+ (void)putImageToUrl:(NSString *)requestUrl bg:(BOOL)bg imgData:(NSData *)imgData succeed:(void (^)(id responseObject))succeed  fail:(void (^)(void))fail{
    [self uploadImgToUrlWithAuth:requestUrl bg:bg imgData:imgData method:@"PUT" succeed:^(id responseObject) {
        if (isNull(responseObject) == YES) {
            fail();
            if (bg == NO) {
                showAndHideWrongJuHua(@"返回数据格式错误",[rootDelegate topViewController].view);
...
+ (void)uploadImgToUrlWithAuth:(NSString *)requestUrl bg:(BOOL)bg imgData:(NSData *)imgData method:(NSString *)method succeed:(void (^)(id responseObject))succeed  fail:(void (^)(void))fail{
    [self uploadImgToUrl:requestUrl bg:bg needAuthHeader:YES imgData:imgData method:method succeed:^(NSURLResponse *response, id responseObject) {
        if (isNull([responseObject objectForKey:@"ok"]) == false && [[responseObject objectForKey:@"ok"] boolValue] == false) {
            if (bg == NO) {
                showAndHideWrongJuHua(NSLocalizedString([responseObject objectForKey:@"msg"],nil), [rootDelegate topViewController].view);
            }
...
+ (void)uploadImgToUrl:(NSString *)requestUrl bg:(BOOL)bg needAuthHeader:(BOOL)needAuthHeader imgData:(NSData *)imgData method:(NSString *)method succeed:(void (^)(NSURLResponse *response, id responseObject))succeed fail:(void (^)(void))fail{
    if (needAuthHeader == YES) {
        //uct to nsdate
        NSString *expire = getUserTokenExpire;
        if (isNullString(expire) == NO) {
    ...
    requestUrl = [NSString stringWithFormat:@"%@?v=%@",requestUrl,kApiVersion];
    NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] multipartFormRequestWithMethod:method URLString:[requestUrl stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]] parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
