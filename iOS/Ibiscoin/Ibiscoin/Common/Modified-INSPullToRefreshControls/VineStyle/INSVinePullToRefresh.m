...
- (void)handleStateChange:(INSPullToRefreshBackgroundViewState)state {
    if (state == INSPullToRefreshBackgroundViewStateNone || state == INSPullToRefreshBackgroundViewStateTriggered) {
        if (state == INSPullToRefreshBackgroundViewStateTriggered) {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [SimpleAudioPlayer playFile:kRefreshPullSoundFileName volume:1.0 loops:0 withCompletionBlock:nil];
            });
        }
...
//        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
//            [SimpleAudioPlayer playFile:kRefreshReleaseSoundFileName volume:1.0 loops:0 withCompletionBlock:nil];
//        });
