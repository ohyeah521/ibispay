...
@implementation MyRoleCell

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    _cellView = [MyRoleCellView new];
    _cellView.cell = self;
...
@implementation MyRoleCellHeader{
    BOOL _trackingTouch;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _imgAvatar.size = CGSizeMake(kMyRoleCellAvatarHeight, kMyRoleCellAvatarHeight);
    _imgAvatar.origin = CGPointMake(kMyRoleCellAvatarPadding, kMyRoleCellAvatarPadding);
    ...
    avatarBorder.width += 2*CGFloatFromPixel(kMyRoleCellAvatarBorderWidth);
    avatarBorder.centerX -= CGFloatFromPixel(kMyRoleCellAvatarBorderWidth);
    avatarBorder.height += 2*CGFloatFromPixel(kMyRoleCellAvatarBorderWidth);
    avatarBorder.centerY -= CGFloatFromPixel(kMyRoleCellAvatarBorderWidth);
    avatarBorder.borderWidth = 2*CGFloatFromPixel(kMyRoleCellAvatarBorderWidth);
    avatarBorder.borderColor = kMyRoleCellAvatarBorderColor;
    ...
    _imgVerify.size = CGSizeMake(kMyRoleCellVerifyImgWidth, kMyRoleCellVerifyImgWidth);
    _imgVerify.center = CGPointMake(_imgAvatar.right - kMyRoleCellVerifyImgWidth/2+kMyRoleCellAvatarBorderWidth, _imgAvatar.bottom - kMyRoleCellVerifyImgWidth/2+kMyRoleCellAvatarBorderWidth);
    _imgVerify.contentMode = UIViewContentModeScaleAspectFit;
    [self addSubview:_imgVerify];
    
    _lblName = [YYLabel new];
    _lblName.size = CGSizeMake(kMyRoleCellNameWidth, kMyRoleCellCoinNameFontSize);
    _lblName.left = _imgAvatar.right + kMyRoleCellNamePaddingLeft;
    ...
    _lblTag = [YYLabel new];
    _lblTag.size = CGSizeMake(kMyRoleCellTagWidth, kMyRoleCellCoinNameFontSize);
    _lblTag.right = screenWidth-kMyRoleCellTagPaddingRight;
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    _header = [MyRoleCellHeader new];
    [_contentView addSubview:_header];
    
    _lblDetail = [YYLabel new];
    _lblDetail.left = kMyRoleCellContentLeftPadding;
    _lblDetail.width = kMyRoleCellContentWidth;
    ...
    _lblAttr1.left = kMyRoleCellContentLeftPadding;
    _lblAttr1.width = kMyRoleCellContentWidth;
    ...
    _lblAttr2.left = kMyRoleCellContentLeftPadding;
    _lblAttr2.width = kMyRoleCellContentWidth-_button.width;
    ...
    _lblTime.left = kMyRoleCellContentLeftPadding;
    _lblTime.width = kMyRoleCellContentWidth-_button.width;
    ...
    _picBGView.width = kMyRoleCellContentWidth;
    _picBGView.backgroundColor = kColorGray4;
    _picBGView.x = kMyRoleCellContentLeftPadding;
        ...
        if (i >= picsCount) {
            ...
            switch (picsCount) {
                case 1: {
                    origin.x = 0;
                    origin.y = 0;
                    picSize.width = kMyRoleCellContentWidth;
                } break;
                case 2:  case 4:{
                    origin.x = (i % 2) * (picSize.width + kMyRoleCellPaddingPic);
                    origin.y = (int)(i / 2) * (picSize.height + kMyRoleCellPaddingPic);
                } break;
                default: {
                    origin.x = (i % 3) * (picSize.width + kMyRoleCellPaddingPic);
                    origin.y = (int)(i / 3) * (picSize.height + kMyRoleCellPaddingPic);
                } break;
            }
